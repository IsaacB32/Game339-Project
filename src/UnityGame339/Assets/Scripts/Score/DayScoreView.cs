...
public class DayScoreView : MonoBehaviour
{
    ...
    [SerializeField] private float _stampDelay = 0.5f;
    ...
    private void UpdateGrade(ScoreService.Grade grade)
    {
        ...
        StartCoroutine(PlayStampDelayed());
    }

    private IEnumerator PlayStampDelayed()
    {
        yield return new WaitForSeconds(_stampDelay);
        _stampObject.SetActive(true);
        _stampAnimator.SetTrigger(StartHash);
    }
}